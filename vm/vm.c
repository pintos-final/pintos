...
bool vm_alloc_page_with_initializer(enum vm_type type, void* upage, bool writable, vm_initializer* init, void* aux)
{
    ...
    if (spt_find_page(spt, upage) == NULL) {
        ...
        /* 3. VM 타입에 따라 적절한 initializer 선택
         *    - 이 initializer는 나중에 page fault 시 호출됨
         *    - page->operations를 설정하는 역할 */
        ...
        /* 4. UNINIT 페이지로 초기화
         *    - page->operations = &uninit_ops 설정
         *    - page->uninit에 init, aux, type, page_initializer 저장 */
...
{
    /* 임시 페이지 생성 및 가상 주소 설정 */
    struct page p;
    p.va = pg_round_down(va); // va를 페이지 경계로 내림

    /* 해시 테이블에서 페이지 검색 */
    struct hash_elem* elem = hash_find(&spt->pages, &p.elem);

    /* 페이지를 찾은 경우 해당 페이지 반환, 못 찾으면 NULL */
    return elem != NULL ? hash_entry(elem, struct page, elem) : NULL;
}

/* Insert PAGE into spt with validation. */
bool spt_insert_page(struct supplemental_page_table* spt, struct page* page)
{
    struct hash_elem* succ = hash_insert(&spt->pages, &page->elem);
    return succ == NULL;
}
...
bool vm_try_handle_fault(struct intr_frame* f, void* addr, bool user, bool write, bool not_present)
{
    ...
    /* not_present가 true면 페이지 없음 (lazy load 대상) */
    if (!not_present)
        return false;
...
bool supplemental_page_table_copy(struct supplemental_page_table* dst, struct supplemental_page_table* src)
{
    ...
    while (hash_next(&i)) {
        ...
        /* 2. 이미 초기화된 페이지인 경우 (ANON 또는 FILE) */
            ...
            /* 즉시 claim하여 물리 프레임 할당 */
            if (!vm_claim_page(upage))
                return false;

            /* 부모 페이지 내용을 자식 페이지로 복사 */
